...
import XCTest
 ...
 extension FeedStoreSpecs where Self: XCTestCase {
 	...
 	func expect(_ sut: FeedStore, toRetrieve expectedResult: FeedStore.RetrievalResult, file: StaticString = #filePath, line: UInt = #line) {
        ...
        case let (.success(.some(expectedFeed)), .success(.some(retreivedFeed))):
            XCTAssertEqual(retreivedFeed.feed, expectedFeed.feed, file: file, line: line)
            XCTAssertEqual(retreivedFeed.timestamp, expectedFeed.timestamp, file: file, line: line)
